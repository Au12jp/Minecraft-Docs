...
# Prepare a clean worktree for the target branch (no conflicts with scripts/)
...
if git ls-remote --heads origin "$BRANCH" | grep -q "$BRANCH"; then
  ...
  # base on current HEAD (usually main)
  git worktree add --force -B "$BRANCH" "$WT_DIR" HEAD
fi
...
if [[ "$(jq -r --arg T "$WANT_TAG" '[.[]|select(.name==$T)]|length' <<<"$tags_json")" -gt 0 ]]; then
  ...
  choose_nearest_strict() { jq -r -f "$JQ_FILTER" --arg ver "$BDS_VER" --argjson want_prev "$WANT_PREV" --argjson strict true  <<<"$tags_json"; }
  choose_nearest_any()    { jq -r -f "$JQ_FILTER" --arg ver "$BDS_VER" --argjson want_prev "$WANT_PREV" --argjson strict false <<<"$tags_json"; }
